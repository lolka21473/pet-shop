...
program
  ...
  .action(async (files: string[], options, cmd) => {
    ...
    let images = newFiles.filter(val => path.extname(val) === extension);
    let imagesTemp = [];

    for (let i = 0; i < images.length; i++) {
      imagesTemp.push("assets/" + i + ".png");
    }
    ...
    for (let i = 0; i < SIZE; i++) {
      ...
      console.log(colors.cyan(`Processing file: ${index} ----------------------------------------------------------------------------------------------------------------------------------------------`)); // outputs green text
        ...
        if (!link) {
          ...
          try {
            ...
            if (metadataFile?.transactionId) {
              ...
              console.log(colors.green(`File uploaded SUCCESSFULLY: ${link}`));
              console.log(colors.cyan(`File: ${index} Procesed -------------------------------------------------------------------------------------------------------------------------------------------`)); // outputs green text
              console.log("")
            }
          ...
          async allIndexesInSlice => {
            for (let offset = 0; offset < allIndexesInSlice.length; offset += 10) {
              ...
              if (onChain.length != indexes.length) {
                ...
                const txId = await anchorProgram.rpc.addConfigLines(ind, indexes.map(i => ({
                ...
                const configInstance = await solConnection.getAccountInfo(
                ...
                indexes.forEach(i => {
                  ...
                  /*
                  console.log(colors.yellow('Verifying file ' + i + "...")); // outputs green text
                  const key = images[i].replace(extension, '').split('/').pop()
                  const thisSlice = configInstance.data.slice(
                  ...
                  if (!name.match(cacheItem.name) || !uri.match(cacheItem.link)) {
                    ...
                    console.log(colors.green('Name', name, 'with', uri, 'VERIFIED')); // outputs green text
                    console.log("")
                  }
    ...
    console.log(colors.green("Upload completed successfully")); // outputs green text
    console.log(colors.yellow("ConfigId: " + config)); // outputs green text
...
program
  ...
  .action(async (directory, cmd) => {
    ...
    console.log(colors.green(`Candy Machine Created Successfully: `));
    console.log({
      candyMachine,
      wallet: walletKey.publicKey,
      config: config,
...
program
  ...
  .action(async (directory, second, options) => {
    ...
    if (verified === keys.length) {
      console.log(colors.green("ALL FILES VERIFIED"));
    }
  });
